# Write the main JavaScript code for app logic (celebrity + style + theme sample data, UI logic, prompt creation)
js_content = '''// ===============================
...
// Save and display saved prompts (localStorage)
document.getElementById('saveAllBtn').onclick = () => {
  let prompts = Array.from(document.querySelectorAll('.code-prompt code')).map(c => c.textContent);
  if(nav...